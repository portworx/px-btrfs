			    ...
			    u64 *wp_ret)
{
	...
	if (empty[0] && empty[1]) {
		...
		*wp_ret = zones[0].start << SECTOR_SHIFT;
		...
		/* Kernel RHEL10-6.12: bd_inode->i_mapping replaced with bd_mapping */
...
int btrfs_get_dev_zone_info(struct btrfs_device *device, bool populate_cache)
{
	...
	//JAR struct request_queue *queue = bdev_get_queue(bdev);
	...
	max_active_zones = 0; //JAR queue_max_active_zones(queue);
	/*
