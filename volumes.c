...
void btrfs_free_device(struct btrfs_device *device)
{
	...
	//JAR   bio_put(device->flush_bio);
	if (unlikely(bio_flagged(device->flush_bio, BIO_REFFED))) {
	  BUG_ON(!atomic_read(&device->flush_bio->__bi_cnt));
	  if (!atomic_dec_and_test(&device->flush_bio->__bi_cnt))
	    return;
        }
	bio_uninit(device->flush_bio);
        kfree(device->flush_bio);
...
static void update_dev_time(const char *device_path)
{
	...
	//JAR now = current_time(d_inode(path.dentry));
	//JAR	inode_update_time(d_inode(path.dentry), &now, S_MTIME | S_CTIME);
	path_put(&path);
}
					...
					const u8 *uuid)
{
	...
	if (!dev->flush_bio) {
        ...
        bio_init(dev->flush_bio, NULL, NULL, 0,0);  //JAR
        dev->flush_bio->bi_pool = NULL;   //JAR
						  ...
						  u8 *fsid)
{
	...
	// JAR ret = open_fs_devices(fs_devices, FMODE_READ, fs_info->bdev_holder);
