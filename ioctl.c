				   ...
				   struct btrfs_qgroup_inherit *inherit)
{
	...
	dentry = lookup_one_len(name, parent->dentry, namelen);  //JAR
					     ...
					     bool destroy_v2)
{
	...
	dentry = lookup_one_len(subvol_name, parent, subvol_namelen); //JAR
	if (IS_ERR(dentry)) {
		err = PTR_ERR(dentry);
		goto out_unlock_dir;
	}
...
static int btrfs_ioctl_encoded_write(struct file *file, void __user *argp, bool compat)
{
	...
	if (memchr_inv(args.reserved, 0, sizeof(args.reserved)))
		goto out_acct;
	if (args.compression == 0 &&   args.encryption == 0)
		goto out_acct;
	/*
